<template>
  <div class="one_publication">
    ...
    <form class="Publication" v-if="update">
      ...
      <p>
        <input
          type="text"
          id="update_titre"
          class="commentaire_input"
          v-model="publications.titre"
        />
      </p>

      <!-- update du corps de message -->
      <p class="parametre">
        <label for="update_corps_message"
          >Améliorer le corps de la publication :</label
        >
      </p>
      <p>
        <input
          type="text"
          id="update_corps_message"
          class="commentaire_input"
    ...
    <p>
      <button class="btn_envoie" v-if="update" @click="update = false">
    ...
    <p>
      <button
        class="btn_envoie"
        v-if="update"
        @click.prevent="updatePublication"
      >
...
export default {
  ...
  methods: {
    getOnePublication() {
      ...
      axios
        .get(`http://localhost:3000/api/publication/${id_publication}`, {
          headers: {
        ...
        .then((response) => {
          ...
          if (
            this.$user.id === this.publications.id_user ||
            this.$user.roleAdmin == 1
          ) {
    ...
    deletePublication() {
      const id_publication = this.$route.params.id;
      axios
        .delete(`http://localhost:3000/api/publication/${id_publication}`, {
          headers: {
            authorization: "Bearer: " + this.$token,
          },
        })
    ...
    deleteCommentary(commentaryId) {
      axios
        .delete(
          `http://localhost:3000/api/publication/commentary/${commentaryId}`,
          {
            headers: {
              authorization: "Bearer: " + this.$token,
              "Content-Type": "application/json",
